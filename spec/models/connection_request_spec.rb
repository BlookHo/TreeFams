...
RSpec.describe ConnectionRequest, :type => :model   do #   , focus: true  ,:disabled=>true
  ...
  describe '- CHECK in ConnectedUser Model methods'   do  # , focus: true
    ...
    let(:connected_users) { current_user_1.connected_users }  # [1,2]
    ...
    describe ' Check ConnectionRequest create_requests Method:'   do  # , focus: true

        let(:current_user_3) { User.third }  # User = 3. Tree = [3]. profile_id = 22
        let(:with_whom_connect_ids) { [3]  }
        let(:max_connection_id) { 444 }
        let(:request_user_id) { 4 }  # User = 2 . Tree = [1,2]. profile_id = 11
        let(:who_connect_ids) { User.find(request_user_id).connected_users }  # User = 2. Tree = [1,2]. profile_id = 11
        let(:certain_koeff_for_connect) { WeafamSetting.first.certain_koeff }  # 4

        before {
          puts "Check User.third  = #{current_user_3.id}, profile_id = #{current_user_3.profile_id} \n"  # user_id = 2
          puts "Check User.third.connected_users who_connect_ids = #{who_connect_ids} \n"  # user_id = 2
          ConnectionRequest.create_requests(who_connect_ids, with_whom_connect_ids, max_connection_id, request_user_id )
        ...
        context '- Check ConnectionRequest AFTER <create_requests> - '   do
          ...
          it '- check ConnectionRequest Third row - made by Method Search - Ok'  do # , focus: true
            ...
            expect(conn_request).to eq( {"id"=>5, "user_id"=>4, "with_user_id"=>3, "confirm"=>nil,
        ...
        context '- Check SearchResults after <start_search> - '   do  #  focus: true
          describe '- check ConnectionRequest have rows count AFTER <create_requests> - Ok'   do
            let(:rows_qty) {5}
            it_behaves_like :successful_connection_request_rows_count
          end
    ...
    describe ' Check ConnectionRequest BEFORE make_request(current_user, with_user_id) Method:'    do  # , focus: true
      ...
      let(:request_user) { User.find(request_user_id) }  # User = 2. Tree = [1,2]. profile_id = 11
      ...
      describe ' Check ConnectionRequest AFTER make_request(current_user, with_user_id) Method:'    do  # , focus: true
        ...
        context '- Check ConnectionRequest AFTER <create_requests> - '    do
          ...
          it '- check ConnectionRequest Third row - made by Method Search - Ok'  do # , focus: true
            conn_request = ConnectionRequest.find(3).attributes.except('created_at','updated_at')
            expect(conn_request).to eq( {"id"=>3, "user_id"=>2, "with_user_id"=>3, "confirm"=>nil,
                                         "done"=>false, "connection_id"=>3} )
          end
        end
