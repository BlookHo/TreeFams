class SearchWork
  ...
  #   check only exclusions
  def self.add_hash_exclusions_check(add_connection_hash)#, certain_koeff)
    checked_add_hash = {}
    ...
    add_connection_hash.each do |profile_searched, profile_found|
      ...
      # NEW LAST VER: Здесь вместо этого SearchCircles.compare_profiles -
      # метод проверки исключений в пересекшихся отношениях
      equality = SearchCircles.compare_profiles_exclusions(profile_searched, profile_found)
      puts "After check_exclusions:  equality = #{equality.inspect}"

      if equality # equality == true -> profiles ARE EQUAL
        checked_add_hash.merge!(profile_searched => profile_found)
        puts "profiles: #{profile_searched} and #{profile_found} -  ARE equal"
      else
        puts "profiles: #{profile_searched} and #{profile_found} - are NOT equal"
