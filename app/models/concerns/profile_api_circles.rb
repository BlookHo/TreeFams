module ProfileApiCircles
  ...
  def old_circles(current_user = nil, max_distance = 3)
    @results = []
    @max_distance = max_distance
    @current_distance = 1
  ...
  def old_collect_circles(user_ids: user_ids, circle: circle, except_ids: except_ids, current_user: current_user, current_distance: current_distance )
    # return if current_distance >= @max_distance
    current_distance += 1
    circle.each do |key|
      current_circle = get_circle(user_ids: user_ids, profile_id: key.is_profile_id, except_ids: except_ids)
  ...
  def old_get_circle(user_ids: user_ids, profile_id: profile_id, except_ids: [])
    ProfileKey.where(user_id: user_ids, profile_id: profile_id)
    .where("relation_id < ?", 9)
    .where.not(is_profile_id: except_ids)
    .order('relation_id')
