module ConnectionRequestsHelper
  ...
  def count_new_requests  #
    @new_requests_count = ConnectionRequest.where(:user_id => current_user.id, :done => false ).select(:created_at).distinct.order('created_at').reverse_order.count if current_user
